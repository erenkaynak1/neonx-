...
function stopWatch(){try{unsubRoom&&unsubRoom()}catch{}try{unsubActions&&unsubActions()}catch{}unsubRoom=unsubActions=null;if(pollTimer){clearInterval(pollTimer);pollTimer=null}lastAppliedRemoteState='';processedActions.clear();fakeGuests.forEach(c=>{try{c.__silentClose=true;c.close()}catch{}});fakeGuests.clear()}
...
function applyRemoteState(snapshot){if(!snapshot)return false;let signature='';try{signature=JSON.stringify(snapshot)}catch{}if(signature&&signature===lastAppliedRemoteState)return false;lastAppliedRemoteState=signature;applyLobbyState(snapshot);return true}
function applyRoomRecord(raw){if(!raw)return;const r=RT.decodeRoom(raw);if(r.status==='closed'){if(!O().isHost)handleClosed();return}room=r;if(!O().isHost&&r.state)applyRemoteState(r.state)}
