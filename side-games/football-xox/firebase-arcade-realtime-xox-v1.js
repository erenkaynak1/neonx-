...
const ready=Promise.all([
  ...
  async function createRoom({gameType,code,ownerId,members,state,status='waiting',ttlMs=DEFAULT_TTL}){
    const id=roomKey(code),roomRef=roomRefById(id),t=Date.now();
    const result=await dbMod.runTransaction(roomRef,current=>{
      ...
      return {
        ...
        revision:Number(mine?current.revision:0)+1,updated_ms:t,expires_ms:t+ttlMs,created_ms:mine?Number(current.created_ms||t):t
      };
    },{applyLocally:false});
    if(!result.committed)throw new Error('room-code-in-use');
    const room=rawRoom(id,result.snapshot.val());
    try{await dbMod.onDisconnect(roomRef).remove()}catch(e){console.warn('XOX disconnect cleanup skipped',e)}
    return room;
  }

  async function updateRoom(roomId,patch={}){
    const roomRef=roomRefById(roomId),snap=await dbMod.get(roomRef);if(!snap.exists())throw new Error('room-not-found');
    ...
    await dbMod.update(roomRef,data);return rawRoom(roomId,{...current,...data});
  }

  async function sendAction({roomId,roomCode,gameType,senderId,action}){
    const actionsRef=dbMod.ref(db,`rooms/${roomId}/arcadeActions`),actionRef=dbMod.push(actionsRef),created=Date.now();
    ...
    actionRooms.set(actionRef.key,roomId);await dbMod.set(actionRef,rec);return{id:actionRef.key,...rec};
  }

  function subscribeRoom(roomId,callback){const roomRef=roomRefById(roomId);return dbMod.onValue(roomRef,snap=>callback({id:roomId,type:snap.exists()?'update':'delete',data:snap.exists()?rawRoom(roomId,snap.val()):null}))}
  function subscribeActions(roomId,callback){const actionsRef=dbMod.ref(db,`rooms/${roomId}/arcadeActions`);return dbMod.onChildAdded(actionsRef,snap=>{actionRooms.set(snap.key,roomId);callback({type:'create',data:{id:snap.key,...snap.val()}})})}
  async function listActions(roomId,sinceMs=0){const snap=await dbMod.get(dbMod.ref(db,`rooms/${roomId}/arcadeActions`)),out=[];snap.forEach(child=>{const v=child.val();if(Number(v?.created_ms||0)>Number(sinceMs||0)){actionRooms.set(child.key,roomId);out.push({id:child.key,...v})}});return out.sort((a,b)=>Number(a.created_ms||0)-Number(b.created_ms||0))}
  async function deleteAction(id){const roomId=actionRooms.get(String(id));if(!roomId)return null;actionRooms.delete(String(id));try{await dbMod.remove(dbMod.ref(db,`rooms/${roomId}/arcadeActions/${id}`));return true}catch{return null}}
  async function closeRoom(roomId){try{await dbMod.update(roomRefById(roomId),{status:'closed',updated_ms:Date.now(),expires_ms:Date.now()+60000});return true}catch{return null}}
...
window.NXXoxTransport='firebase-realtime-v1';
})();
