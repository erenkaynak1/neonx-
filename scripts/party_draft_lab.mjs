...
async function launchDraftFromParty(leader,member,partyId){
  ...
  await leader.page.waitForURL(url=>new URL(url).searchParams.get('nxBroker')==='1',{timeout:TIMEOUT});const host=launchParams(leader.page);assert.equal(host.nxParty,partyId);assert.equal(host.nxRole,'host');assert.equal(host.nxAuto,'1');assert.equal(host.nxPartySize,'2');assert.ok(host.nxCode);assert.ok(host.nxMatch);mark('Lider broker URL ile Drafta yönlendi',{host});
  await leader.page.waitForSelector('#draftScreen',{state:'attached',timeout:TIMEOUT});mark('Lider Draft ekranı yüklendi');
  await member.page.waitForURL(url=>new URL(url).searchParams.get('nxBroker')==='1',{timeout:TIMEOUT});const guest=launchParams(member.page);assert.equal(guest.nxParty,partyId);assert.equal(guest.nxRole,'guest');assert.equal(guest.nxAuto,'1');assert.equal(guest.nxCode,host.nxCode,'Parti üyeleri farklı Draft odasına gitti');assert.equal(guest.nxMatch,host.nxMatch,'Parti üyeleri farklı match kimliği aldı');assert.equal(guest.nxOpponent,leader.uid);assert.equal(host.nxOpponent,member.uid);mark('Üye aynı broker odasına otomatik çekildi',{guest});
  await member.page.waitForSelector('#draftScreen',{state:'attached',timeout:TIMEOUT});mark('Üye Draft ekranı yüklendi');
