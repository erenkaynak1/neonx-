...
# Transfermarkt remains primary. Mackolik is only a secondary verifier/fallback.
...
def main():
    ...
    filled=corroborated=suspicious=0
    ...
    for rid,ref in refs.get('players',{}).items():
        ...
        for field in ('career_appearances','career_goals','career_assists'):
            ...
            if delta<=TOL[field]:
                corroborated+=1
                # Individual Mackolik profile is used as the fresher counter when the
                # two Transfermarkt/Mackolik values already agree within a tiny margin.
                p[field]=int(rv)
            else:
                conflicts.append({
                    'field':field,
    ...
    playable=[];remaining=[]
    for p in rows:
        p['playable']=(not p.get('secondary_conflict')) and all(p.get(k) is not None for k in req) and int(p.get('career_appearances') or 0)>0
    ...
    print(json.dumps({'playable':len(playable),'filled':filled,'corroborated':corroborated,'conflicts':suspicious},ensure_ascii=False))
