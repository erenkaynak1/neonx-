...
def download():
    if TMP.exists() and TMP.stat().st_size>100_000_000:
        return
    headers={'User-Agent':'Mozilla/5.0 NEON-XI-Career-Twin/2.0'}
    with requests.get(ARCHIVE_URL,stream=True,timeout=(30,240),headers=headers) as r:
        r.raise_for_status()
