...
ROOT=Path(__file__).resolve().parents[1]; DATA=ROOT/'side-games'/'career-twin'/'data'; UA={'User-Agent':'NEON-XI-Career-Twin/1.3'}
...
def iv(v):
 ...
 s=str(v).strip().replace(' ',''); m=re.search(r'-?\d+(?:[.,]\d+)?',s); return int(round(float(m.group().replace(',','.')))) if m else None

def main():
 df=pd.read_csv(io.BytesIO(requests.get(URL,headers=UA,timeout=90).content),low_memory=False)
 mp={}
 for _,x in df.iterrows():
  k=norm(x.get('nom'))
  if k:mp[k]={'trophies':iv(x.get('trofeus')),'career_appearances':iv(x.get('partits')),'career_goals':iv(x.get('gols')),'career_assists':iv(x.get('assistencies')),'uoc_team':None if pd.isna(x.get('equip')) else str(x.get('equip')),'uoc_age':iv(x.get('edat'))}
 ...
 for p in cand:
  ...
  if u:
   ...
   p.setdefault('sources',{})['career_stats']='UOC-Transfermarkt-2026' if all(p.get(k) is not None for k in ('career_appearances','career_goals','career_assists')) else p.get('sources',{}).get('career_stats')
 ...
 meta=json.loads((DATA/'meta.json').read_text());meta.update({'playable_count':len(playable),'candidate_count':len(remain),'uoc_file':'jugadors.csv','uoc_matched_names':matched});(DATA/'meta.json').write_text(json.dumps(meta,ensure_ascii=False,indent=2))
 print(json.dumps({'matched':matched,'playable_total':len(playable)},ensure_ascii=False))
