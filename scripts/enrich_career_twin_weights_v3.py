...
def forms(s):
 ...
 if len(ts)>=2:
  out.update((''.join(ts),ts[0]+ts[-1],ts[-1]+ts[0]))
...
def main():
 ...
 fc['nclub']=fc.club_name.fillna('').map(norm);fc['nshort']=fc.short_name.fillna('').map(norm);fc['nlong']=fc.long_name.fillna('').map(norm)
 fc['forms']=fc.apply(lambda r:forms(r.get('short_name'))|forms(r.get('long_name')),axis=1)
 filled=0;exact_context=0;multikey=0
 for p in cand:
  if p.get('weight_kg') is not None or p.get('trophies') is None:continue
  h=p.get('height_cm');age=p.get('uoc_age') or age_from_birth(p.get('birth_date'));club=norm(p.get('uoc_team') or p.get('club'));pname=norm(p.get('name'));pforms=forms(p.get('name'))
  q=fc
  ...
  if len(erows)==1:
   chosen=erows[0];multikey+=1
  elif len(erows)>1:
   es=[]
   for idx in erows:
    r=fc.loc[idx];cs=WRatio(club,r.nclub) if club and r.nclub else 0;hd=abs(float(r.height_cm)-float(h)) if h is not None else 99;ad=abs(float(r.age)-float(age)) if age is not None else 99
    es.append((cs-hd*4-ad*2,idx))
  ...
  if chosen is None:
   ...
   scored=[]
   for idx,r in q.iterrows():
    ns=max(WRatio(pname,r.nshort),WRatio(pname,r.nlong));cs=WRatio(club,r.nclub) if club and r.nclub else 0;hd=abs(float(r.height_cm)-float(h)) if h is not None else 99;ad=abs(float(r.age)-float(age)) if age is not None else 99
    score=ns+(10 if cs>=92 else 6 if cs>=80 else 2 if cs>=65 else 0)+(5 if hd<=1 else 3 if hd<=2 else 1 if hd<=3 else 0)+(3 if ad<=1 else 1 if ad<=2 else 0)
    scored.append((score,ns,cs,hd,ad,idx))
   scored.sort(reverse=True)
   if scored:
    best=scored[0];second=scored[1][0] if len(scored)>1 else 0
    strong_name=best[1]>=91 and best[3]<=3 and best[4]<=2 and best[0]-second>=3
    strong_context=best[1]>=82 and best[2]>=90 and best[3]<=2 and best[4]<=1 and best[0]-second>=4
