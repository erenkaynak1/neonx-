...
async function currentPartyMembership(){const uid=state.user?.uid;if(!uid)return '';const pointer=ref(db,`social/userParty/${uid}`),id=String((await get(pointer)).val()||'');if(!id)return '';const party=(await get(ref(db,`social/parties/${id}`))).val();if(party?.members?.[uid])return id;await remove(pointer);return ''}
async function createParty(){const existing=await currentPartyMembership();if(existing){message('Zaten aktif bir partidesin.');return existing}const id=`p_${state.user.uid.slice(0,8)}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,6)}`;await update(ref(db),{[`social/parties/${id}`]:{leaderUid:state.user.uid,createdAt:serverTimestamp(),members:{[state.user.uid]:{username:state.profile.username,joinedAt:serverTimestamp()}}},[`social/userParty/${state.user.uid}`]:id});message('Kalıcı parti kuruldu.');return id}
