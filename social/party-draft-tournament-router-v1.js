...
import {getDatabase,get,ref,runTransaction,serverTimestamp,update} from 'https://www.gstatic.com/firebasejs/12.16.0/firebase-database.js';
...
async function createTournamentRoom(user,name,size){
  for(let attempt=0;attempt<50;attempt++){
    ...
    const tx=await runTransaction(rr,current=>{
      ...
      return {version:'2.0-online-tournament',mode:'tournament',status:'lobby',size,hostUid:user.uid,createdAt:now,updatedAt:now,players:{[user.uid]:{uid:user.uid,name,teamIndex:0,connected:true,joinedAt:now}}};
    },{applyLocally:false});
...
async function launchTournament(button){
  ...
  try{
    ...
    const partyId=String((await get(ref(db,`social/userParty/${user.uid}`))).val()||'');if(!partyId)throw new Error('Aktif parti bulunamadı.');
    const party=(await get(ref(db,`social/parties/${partyId}`))).val();if(!party)throw new Error('Parti artık mevcut değil.');if(party.leaderUid!==user.uid)throw new Error('Turnuvayı yalnızca parti lideri başlatabilir.');
    const members=Object.keys(party.members||{});if(members.length<=2)throw new Error('Turnuva yönlendirmesi için en az 3 oyuncu gerekli.');if(members.length>8)throw new Error('NEON XI turnuvası en fazla 8 parti oyuncusunu destekliyor.');
    ...
    const tournamentSize=members.length<=4?4:8,name=party.members?.[user.uid]?.username||window.NEON_SOCIAL?.profile?.username||'NEON Oyuncu';status(`${members.length} kişilik parti algılandı. ${tournamentSize}'li turnuva hazırlanıyor…`);
    const roomCode=await createTournamentRoom(user,name,tournamentSize),nonce=`${Date.now()}_${Math.random().toString(36).slice(2,8)}`,matchId=`pt_${partyId}_${nonce}`,roles=Object.fromEntries(members.map(uid=>[uid,uid===user.uid?'host':'guest']));
    ...
    const opponent=members.find(uid=>uid!==user.uid)||'',target=new URL('index.html',base),params={nxParty:partyId,nxLaunch:nonce,nxAuto:'1',nxTournament:'1',nxTournamentSize:tournamentSize,nxRole:'host',nxCode:roomCode,nxPartySize:members.length,nxName:name,nxUid:user.uid,nxOpponent:opponent,nxMatch:matchId};
    Object.entries(params).forEach(([k,v])=>target.searchParams.set(k,String(v)));status('Turnuva odası hazır. Parti turnuvaya aktarılıyor…');location.href=target.href;return true;
  }catch(e){console.error('[NEON XI] party tournament routing failed',e);status(e?.message||'Turnuva başlatılamadı.',true);if(button)button.disabled=false;throw e}
}
...
document.addEventListener('click',event=>{
  ...
  const party=window.NEON_SOCIAL?.party,members=Object.keys(party?.members||{});if(members.length<=2)return;
  event.preventDefault();event.stopImmediatePropagation();launchTournament(button).catch(()=>{});
},true);

window.NEON_DRAFT_PARTY_ROUTER={version:'1.1',rule:'2=1v1,3-4=4-team,5-8=8-team',launchTournament};
